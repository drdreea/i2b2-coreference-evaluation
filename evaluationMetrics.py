...
#                        self.forcedExiting()
    ...
    def partitionOverlap(self, goldStandardPartition, systemPartition):
        ...
        for goldStandardMarkable in goldStandardPartition:
            for systemMarkable in systemPartition:
                if goldStandardMarkable.startPos == systemMarkable.startPos and \
                goldStandardMarkable.startWordIndex == systemMarkable.startWordIndex and \
                goldStandardMarkable.endWordIndex == systemMarkable.endWordIndex and \
                goldStandardMarkable.endPos == systemMarkable.endPos:
    ...
    def getPartitions(self, markable):
        ...
        for chain1 in self.systemPredictions:
            for chainMarkable in chain1:
                if markable.uniqueId == chainMarkable.uniqueId :
...
#          
    ...
    def markableExactIdentity(self, systemMarkable, gsMarkable):
        ...
        Markables are identical if they have 
            1. the same text (we removed this due to some issues with the ODIE corpus)
            2. the same overlaps 
        ...
        if        systemMarkable.startPos == gsMarkable.startPos and  \
...
#                systemMarkable.text == gsMarkable.text and  \
